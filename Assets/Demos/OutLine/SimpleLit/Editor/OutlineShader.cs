...
{
    internal class OutlineShader : BaseShaderGUI
    {
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            foreach (var property in properties)
            {
                if (property.name.Contains("OutlineWidth"))
                {
                    var guiContent = new GUIContent(property.displayName);
                    materialEditor.ShaderProperty(property, guiContent);
                }
                if (property.name.Contains("FlickerSwitch"))
                {
                    var guiContent = new GUIContent(property.displayName);
                    materialEditor.ShaderProperty(property, guiContent);
                }
                if (property.name.Contains("FlickerFrequency"))
                {
                    var guiContent = new GUIContent(property.displayName);
                    materialEditor.ShaderProperty(property, guiContent);
                }
                if (property.name.Contains("FlickerColor"))
                {
                    var guiContent = new GUIContent(property.displayName);
                    materialEditor.ShaderProperty(property, guiContent);
                }
