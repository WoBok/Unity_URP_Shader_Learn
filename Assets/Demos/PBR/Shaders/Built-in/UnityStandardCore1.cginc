...
#include "UnityStandardInput1.cginc"
#include "UnityPBSLighting1.cginc"
...
float GGX(float roughness, float ndoth) {
    ...
    return a2 / max(UNITY_PI * d * d, NON_ZERO);
}
...
half4 PBS(half3 diffColor, half3 specColor, half smoothness,
...
UnityLight light) {
    ...
    half roughness = smoothness;

    half3 diffuseTerm = DisneyDiffuse(roughness, diffColor, ndotv, ndotl, ldoth) * ndotl;

   roughness = max(roughness * roughness, 0.002);
    half3 F = Fresnel(specColor, ldoth);
    half G = SmithJointGGX(roughness, ndotl, ndotv);
    half D = GGX(roughness, ndoth);
    half3 specularTerm = (F * D * G) * UNITY_PI;/// (4 * ndotl * ndotv);//
    specularTerm = max(0, specularTerm * ndotl);

    half3 lightColor = half3(1,1,1);

    half3 color = diffuseTerm * lightColor + specularTerm * lightColor;
    return half4(color, 1);
}
...
half4 fragForwardBaseInternal(VertexOutputForwardBase i) {
    ...
    half4 c = PBS(s.diffColor, s.specColor,  s.smoothness, s.normalWorld, -s.eyeVec, gi.light);// half4(1,1,1,1);//
